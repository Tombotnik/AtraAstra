...
class MPU6050:
   ...
   def readDataFromFifo(self, packageSize=24):
      if self.fifoCount == 0 :
   ...
   def convertData(self,ListData):
      ShortData = struct.unpack(">"+"h"*14, memoryview(bytearray(ListData)))
   ...
   def enableFifo(self,flag, byteValue=0x00000000):
      bus.write_byte_data(self.MPU6050_ADDRESS, self.MPU6050_RA_FIFO_EN,0)
      if flag:
        self.resetFifo()
        bus.write_byte_data(self.MPU6050_ADDRESS, self.MPU6050_RA_FIFO_EN,byteValue)
